...
int main(int argc, char* argv[]) { //To run the program, user must enter ./test and the number of ships they'd like to play with.
	//welcomeMsg();	
	if (argc != 3) { //If the user does not enter 3 arguments into the command line, the below error message will display.
		std::cout << "Invalid number of parameters! To run the program, enter ./test followed by the number of desired ships (1, 2, 3, 4, or 5). for two players\n";
	} else { //When the user has etered ./test and a second argument into the command line, they enter the below if/else loop.
		if (atoi(argv[1]) < 1 || atoi(argv[1]) > 5||atoi(argv[2]) < 1 || atoi(argv[2]) > 5) { //If the 2nd and 3rd argument converted to an integer is not between 1 and 5 inclusive, display error message.
			...
			int opShips = atoi(argv[2]);
			std::cout << "\nNumber of ships selected for Player 1: " << numShips << "\n"; //Displays number of ships the first player has selected.
			std::cout << "\nNumber of ships selected for Player 2: " << opShips << "\n"; //Displays number of ships the second player has selected.

			Executive exec(numShips, opShips); //Creates an instance of the Executive class called exec, with numShips passed in as the argument.
