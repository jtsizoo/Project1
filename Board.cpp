...
Board::Board(int size) { //Constructor for Board class, passes in parameters: size of board (always 10x10) and string representing player name/#.
...
bool Board::insertShip(int size, int row, int col, bool dir) { //insertShip takes in size, row, col, and dir to produce Boolean logic on whether ship can be inserted.
    int x = col; //Set integer x equal to parameter integer col
    int y = row; //Set integer y equal to parameter integer row
    if (dir == true) { //If the player desires to orient their ship in the horizontal direction...
    ...
    for (int i = 0; i < size; i++) { //For loop traverses size of current ship looking to be placed.
        if (placeGrid[y][x] == 'X') { //If within the placeGrid board an [i][j] location already contains an ship
            return false; //Return false, because a ship is already present and anothre cannot be placed.
        } else {
            qDebug() << placeGrid[y][x] << " ";
            placeGrid[y][x] = 'X'; //Else, store the ship location with the ship number.

            if (dir == true) { //If desired horizontal orientation, then increase x by 1.
                x += 1;
